<template>
  <div class="relative">
    <input
      class="bg-purple-white w-full shadow rounded border-0 p-3"
      v-model="text"
  ...
  methods: {
    onInput() {
      ...
      const searchResults = this.items.filter((item) => 
        item[this.searchField].toLowerCase().includes(this.text.toLowerCase()))

      this.$emit('onInput', searchResults)
    }
  }
}
</script>